...
enum {
  ...
  kCARingBufferError_TooMuch = 3,  // fetch start time is earlier than buffer start time and fetch
                                   // end time is later than buffer end time
  kCARingBufferError_CPUOverload = 4  // the reader is unable to get enough CPU cycles to capture a
                                      // consistent snapshot of the time bounds
};
...
class CARingBuffer {
 ...
 protected:
  UInt32 FrameOffset(SampleTime frameNumber) {
