...
#include "Utility/CAMutex.h"
#include "Utility/CARingBuffer.h"
...
//==================================================================================================
#pragma mark -
#pragma mark AudioLoopback State
//==================================================================================================

//	Declare the internal object ID numbers for all the objects this driver implements. Note that
//	this driver has a fixed set of objects that never grows or shrinks. If this were not the
// case, 	the driver would need to have a means to dynamically allocate these IDs. It's
// important to 	realize that a lot of the structure of this driver is vastly simpler when
// the IDs are all 	known a priori. Comments in the code will try to identify some of these
// simplifications and
...
//	multiple devices were supported, this state would need to be encapsulated in one or more
// structs 	so that each object's state can be tracked individually. 	Note also that we
// share a single mutex across all objects to be thread safe for the same reason.
...
static CAMutex gIOMutex("Device IO");
static CARingBuffer gLoopbackRingBuffer;
//==================================================================================================
#pragma mark -
...
extern "C" void* AudioLoopback_Create(CFAllocatorRef inAllocator, CFUUIDRef inRequestedTypeUUID);
static HRESULT AudioLoopback_QueryInterface(void* inDriver, REFIID inUUID, LPVOID* outInterface);
static ULONG AudioLoopback_AddRef(void* inDriver);
static ULONG AudioLoopback_Release(void* inDriver);
static OSStatus AudioLoopback_Initialize(AudioServerPlugInDriverRef inDriver,
                                    AudioServerPlugInHostRef inHost);
static OSStatus AudioLoopback_CreateDevice(AudioServerPlugInDriverRef inDriver,
                                      CFDictionaryRef inDescription,
                                      const AudioServerPlugInClientInfo* inClientInfo,
                                      AudioObjectID* outDeviceObjectID);
static OSStatus AudioLoopback_DestroyDevice(AudioServerPlugInDriverRef inDriver,
                                       AudioObjectID inDeviceObjectID);
static OSStatus AudioLoopback_AddDeviceClient(AudioServerPlugInDriverRef inDriver,
                                         AudioObjectID inDeviceObjectID,
                                         const AudioServerPlugInClientInfo* inClientInfo);
static OSStatus AudioLoopback_RemoveDeviceClient(AudioServerPlugInDriverRef inDriver,
                                            AudioObjectID inDeviceObjectID,
                                            const AudioServerPlugInClientInfo* inClientInfo);
static OSStatus AudioLoopback_PerformDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver,
                                                          AudioObjectID inDeviceObjectID,
                                                          UInt64 inChangeAction,
                                                          void* inChangeInfo);
static OSStatus AudioLoopback_AbortDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver,
                                                        AudioObjectID inDeviceObjectID,
                                                        UInt64 inChangeAction, void* inChangeInfo);
static Boolean AudioLoopback_HasProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID,
                                    pid_t inClientProcessID,
                                    const AudioObjectPropertyAddress* inAddress);
static OSStatus AudioLoopback_IsPropertySettable(AudioServerPlugInDriverRef inDriver,
                                            AudioObjectID inObjectID, pid_t inClientProcessID,
                                            const AudioObjectPropertyAddress* inAddress,
                                            Boolean* outIsSettable);
static OSStatus AudioLoopback_GetPropertyDataSize(AudioServerPlugInDriverRef inDriver,
                                             AudioObjectID inObjectID, pid_t inClientProcessID,
                                             const AudioObjectPropertyAddress* inAddress,
                                             UInt32 inQualifierDataSize,
                                             const void* inQualifierData, UInt32* outDataSize);
static OSStatus AudioLoopback_GetPropertyData(AudioServerPlugInDriverRef inDriver,
                                         AudioObjectID inObjectID, pid_t inClientProcessID,
                                         const AudioObjectPropertyAddress* inAddress,
                                         UInt32 inQualifierDataSize, const void* inQualifierData,
                                         UInt32 inDataSize, UInt32* outDataSize, void* outData);
static OSStatus AudioLoopback_SetPropertyData(AudioServerPlugInDriverRef inDriver,
                                         AudioObjectID inObjectID, pid_t inClientProcessID,
                                         const AudioObjectPropertyAddress* inAddress,
                                         UInt32 inQualifierDataSize, const void* inQualifierData,
                                         UInt32 inDataSize, const void* inData);
static OSStatus AudioLoopback_StartIO(AudioServerPlugInDriverRef inDriver,
                                 AudioObjectID inDeviceObjectID, UInt32 inClientID);
static OSStatus AudioLoopback_StopIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID,
                                UInt32 inClientID);
static OSStatus AudioLoopback_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver,
                                          AudioObjectID inDeviceObjectID, UInt32 inClientID,
                                          Float64* outSampleTime, UInt64* outHostTime,
                                          UInt64* outSeed);
static OSStatus AudioLoopback_WillDoIOOperation(AudioServerPlugInDriverRef inDriver,
                                           AudioObjectID inDeviceObjectID, UInt32 inClientID,
                                           UInt32 inOperationID, Boolean* outWillDo,
                                           Boolean* outWillDoInPlace);
static OSStatus AudioLoopback_BeginIOOperation(AudioServerPlugInDriverRef inDriver,
                                          AudioObjectID inDeviceObjectID, UInt32 inClientID,
                                          UInt32 inOperationID, UInt32 inIOBufferFrameSize,
                                          const AudioServerPlugInIOCycleInfo* inIOCycleInfo);
static OSStatus AudioLoopback_DoIOOperation(AudioServerPlugInDriverRef inDriver,
                                       AudioObjectID inDeviceObjectID,
                                       AudioObjectID inStreamObjectID, UInt32 inClientID,
                                       UInt32 inOperationID, UInt32 inIOBufferFrameSize,
                                       const AudioServerPlugInIOCycleInfo* inIOCycleInfo,
                                       void* ioMainBuffer, void* ioSecondaryBuffer);
static OSStatus AudioLoopback_EndIOOperation(AudioServerPlugInDriverRef inDriver,
                                        AudioObjectID inDeviceObjectID, UInt32 inClientID,
                                        UInt32 inOperationID, UInt32 inIOBufferFrameSize,
                                        const AudioServerPlugInIOCycleInfo* inIOCycleInfo);

//	Implementation
static Boolean AudioLoopback_HasPlugInProperty(AudioServerPlugInDriverRef inDriver,
                                          AudioObjectID inObjectID, pid_t inClientProcessID,
                                          const AudioObjectPropertyAddress* inAddress);
static OSStatus AudioLoopback_IsPlugInPropertySettable(AudioServerPlugInDriverRef inDriver,
                                                  AudioObjectID inObjectID, pid_t inClientProcessID,
                                                  const AudioObjectPropertyAddress* inAddress,
                                                  Boolean* outIsSettable);
static OSStatus AudioLoopback_GetPlugInPropertyDataSize(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32* outDataSize);
static OSStatus AudioLoopback_GetPlugInPropertyData(AudioServerPlugInDriverRef inDriver,
                                               AudioObjectID inObjectID, pid_t inClientProcessID,
                                               const AudioObjectPropertyAddress* inAddress,
                                               UInt32 inQualifierDataSize,
                                               const void* inQualifierData, UInt32 inDataSize,
                                               UInt32* outDataSize, void* outData);
static OSStatus AudioLoopback_SetPlugInPropertyData(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32 inDataSize, const void* inData,
    UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2]);

static Boolean AudioLoopback_HasBoxProperty(AudioServerPlugInDriverRef inDriver,
                                       AudioObjectID inObjectID, pid_t inClientProcessID,
                                       const AudioObjectPropertyAddress* inAddress);
static OSStatus AudioLoopback_IsBoxPropertySettable(AudioServerPlugInDriverRef inDriver,
                                               AudioObjectID inObjectID, pid_t inClientProcessID,
                                               const AudioObjectPropertyAddress* inAddress,
                                               Boolean* outIsSettable);
static OSStatus AudioLoopback_GetBoxPropertyDataSize(AudioServerPlugInDriverRef inDriver,
                                                AudioObjectID inObjectID, pid_t inClientProcessID,
                                                const AudioObjectPropertyAddress* inAddress,
                                                UInt32 inQualifierDataSize,
                                                const void* inQualifierData, UInt32* outDataSize);
static OSStatus AudioLoopback_GetBoxPropertyData(AudioServerPlugInDriverRef inDriver,
                                            AudioObjectID inObjectID, pid_t inClientProcessID,
                                            const AudioObjectPropertyAddress* inAddress,
                                            UInt32 inQualifierDataSize, const void* inQualifierData,
                                            UInt32 inDataSize, UInt32* outDataSize, void* outData);
static OSStatus AudioLoopback_SetBoxPropertyData(AudioServerPlugInDriverRef inDriver,
                                            AudioObjectID inObjectID, pid_t inClientProcessID,
                                            const AudioObjectPropertyAddress* inAddress,
                                            UInt32 inQualifierDataSize, const void* inQualifierData,
                                            UInt32 inDataSize, const void* inData,
...
static Boolean AudioLoopback_HasDeviceProperty(AudioServerPlugInDriverRef inDriver,
                                          AudioObjectID inObjectID, pid_t inClientProcessID,
                                          const AudioObjectPropertyAddress* inAddress);
static OSStatus AudioLoopback_IsDevicePropertySettable(AudioServerPlugInDriverRef inDriver,
                                                  AudioObjectID inObjectID, pid_t inClientProcessID,
                                                  const AudioObjectPropertyAddress* inAddress,
                                                  Boolean* outIsSettable);
static OSStatus AudioLoopback_GetDevicePropertyDataSize(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32* outDataSize);
static OSStatus AudioLoopback_GetDevicePropertyData(AudioServerPlugInDriverRef inDriver,
                                               AudioObjectID inObjectID, pid_t inClientProcessID,
                                               const AudioObjectPropertyAddress* inAddress,
                                               UInt32 inQualifierDataSize,
                                               const void* inQualifierData, UInt32 inDataSize,
                                               UInt32* outDataSize, void* outData);
static OSStatus AudioLoopback_SetDevicePropertyData(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32 inDataSize, const void* inData,
    UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2]);

static Boolean AudioLoopback_HasStreamProperty(AudioServerPlugInDriverRef inDriver,
                                          AudioObjectID inObjectID, pid_t inClientProcessID,
                                          const AudioObjectPropertyAddress* inAddress);
static OSStatus AudioLoopback_IsStreamPropertySettable(AudioServerPlugInDriverRef inDriver,
                                                  AudioObjectID inObjectID, pid_t inClientProcessID,
                                                  const AudioObjectPropertyAddress* inAddress,
                                                  Boolean* outIsSettable);
static OSStatus AudioLoopback_GetStreamPropertyDataSize(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32* outDataSize);
static OSStatus AudioLoopback_GetStreamPropertyData(AudioServerPlugInDriverRef inDriver,
                                               AudioObjectID inObjectID, pid_t inClientProcessID,
                                               const AudioObjectPropertyAddress* inAddress,
                                               UInt32 inQualifierDataSize,
                                               const void* inQualifierData, UInt32 inDataSize,
                                               UInt32* outDataSize, void* outData);
static OSStatus AudioLoopback_SetStreamPropertyData(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32 inDataSize, const void* inData,
    UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2]);

static Boolean AudioLoopback_HasControlProperty(AudioServerPlugInDriverRef inDriver,
                                           AudioObjectID inObjectID, pid_t inClientProcessID,
                                           const AudioObjectPropertyAddress* inAddress);
static OSStatus AudioLoopback_IsControlPropertySettable(AudioServerPlugInDriverRef inDriver,
                                                   AudioObjectID inObjectID,
                                                   pid_t inClientProcessID,
                                                   const AudioObjectPropertyAddress* inAddress,
                                                   Boolean* outIsSettable);
static OSStatus AudioLoopback_GetControlPropertyDataSize(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32* outDataSize);
static OSStatus AudioLoopback_GetControlPropertyData(AudioServerPlugInDriverRef inDriver,
                                                AudioObjectID inObjectID, pid_t inClientProcessID,
                                                const AudioObjectPropertyAddress* inAddress,
                                                UInt32 inQualifierDataSize,
                                                const void* inQualifierData, UInt32 inDataSize,
                                                UInt32* outDataSize, void* outData);
static OSStatus AudioLoopback_SetControlPropertyData(
    AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize,
    const void* inQualifierData, UInt32 inDataSize, const void* inData,
    UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2]);
...
static AudioServerPlugInDriverInterface gAudioServerPlugInDriverInterface = {
    NULL,
    AudioLoopback_QueryInterface,
    AudioLoopback_AddRef,
    AudioLoopback_Release,
    AudioLoopback_Initialize,
    AudioLoopback_CreateDevice,
    AudioLoopback_DestroyDevice,
    AudioLoopback_AddDeviceClient,
    AudioLoopback_RemoveDeviceClient,
    AudioLoopback_PerformDeviceConfigurationChange,
    AudioLoopback_AbortDeviceConfigurationChange,
    AudioLoopback_HasProperty,
    AudioLoopback_IsPropertySettable,
    AudioLoopback_GetPropertyDataSize,
    AudioLoopback_GetPropertyData,
    AudioLoopback_SetPropertyData,
    AudioLoopback_StartIO,
    AudioLoopback_StopIO,
    AudioLoopback_GetZeroTimeStamp,
    AudioLoopback_WillDoIOOperation,
    AudioLoopback_BeginIOOperation,
    AudioLoopback_DoIOOperation,
    AudioLoopback_EndIOOperation};
...
static HRESULT AudioLoopback_QueryInterface(void* inDriver, REFIID inUUID, LPVOID* outInterface) {
  ...
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_QueryInterface: bad driver reference");
  FailWithAction(outInterface == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_QueryInterface: no place to store the returned interface");

  //	make a CFUUIDRef from inUUID
  theRequestedUUID = CFUUIDCreateFromUUIDBytes(NULL, inUUID);
  FailWithAction(theRequestedUUID == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_QueryInterface: failed to create the CFUUIDRef");
...
static ULONG AudioLoopback_AddRef(void* inDriver) {
  ...
  //	check the arguments
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done, "AudioLoopback_AddRef: bad driver reference");
...
static ULONG AudioLoopback_Release(void* inDriver) {
  ...
  //	check the arguments
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done, "AudioLoopback_Release: bad driver reference");
...
static OSStatus AudioLoopback_Initialize(AudioServerPlugInDriverRef inDriver,
  ...
  CAMutex::Locker theIOLocker(gIOMutex);
  gLoopbackRingBuffer.Allocate(1, 2 * sizeof(Float32), kDevice_RingBufferSize);

  //    check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_Initialize: bad driver reference");
...
#pragma unused(inDescription, inClientInfo, outDeviceObjectID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_CreateDevice: bad driver reference");
...
#pragma unused(inDeviceObjectID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_DestroyDevice: bad driver reference");
...
#pragma unused(inClientInfo)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_AddDeviceClient: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_AddDeviceClient: bad device ID");
...
#pragma unused(inClientInfo)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_RemoveDeviceClient: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_RemoveDeviceClient: bad device ID");
...
#pragma unused(inChangeInfo)
  ...
  //    check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_PerformDeviceConfigurationChange: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_PerformDeviceConfigurationChange: bad device ID");
  FailWithAction((inChangeAction != 44100) && (inChangeAction != 48000),
                 theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_PerformDeviceConfigurationChange: bad sample rate");
...
static Boolean AudioLoopback_HasProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID,
  ...
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done,
         "AudioLoopback_HasProperty: bad driver reference");
  FailIf(inAddress == NULL, Done, "AudioLoopback_HasProperty: no address");
  ...
  //	property in the AudioLoopback_GetPropertyData() method.
  switch (inObjectID) {
    case kObjectID_PlugIn:
      theAnswer = AudioLoopback_HasPlugInProperty(inDriver, inObjectID, inClientProcessID, inAddress);
      break;

    case kObjectID_Box:
      theAnswer = AudioLoopback_HasBoxProperty(inDriver, inObjectID, inClientProcessID, inAddress);
      break;

    case kObjectID_Device:
      theAnswer = AudioLoopback_HasDeviceProperty(inDriver, inObjectID, inClientProcessID, inAddress);
      break;

    case kObjectID_Stream_Input:
    case kObjectID_Stream_Output:
      theAnswer = AudioLoopback_HasStreamProperty(inDriver, inObjectID, inClientProcessID, inAddress);
      break;
    ...
    case kObjectID_DataDestination_PlayThru_Master:
      theAnswer = AudioLoopback_HasControlProperty(inDriver, inObjectID, inClientProcessID, inAddress);
      break;
  };
...
static OSStatus AudioLoopback_IsPropertySettable(AudioServerPlugInDriverRef inDriver,
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_IsPropertySettable: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsPropertySettable: no address");
  FailWithAction(outIsSettable == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsPropertySettable: no place to put the return value");
  ...
  switch (inObjectID) {
    case kObjectID_PlugIn:
      theAnswer = AudioLoopback_IsPlugInPropertySettable(inDriver, inObjectID, inClientProcessID,
                                                    inAddress, outIsSettable);
      break;

    case kObjectID_Box:
      theAnswer = AudioLoopback_IsBoxPropertySettable(inDriver, inObjectID, inClientProcessID, inAddress,
                                                 outIsSettable);
      break;

    case kObjectID_Device:
      theAnswer = AudioLoopback_IsDevicePropertySettable(inDriver, inObjectID, inClientProcessID,
                                                    inAddress, outIsSettable);
      break;

    case kObjectID_Stream_Input:
    case kObjectID_Stream_Output:
      theAnswer = AudioLoopback_IsStreamPropertySettable(inDriver, inObjectID, inClientProcessID,
                                                    inAddress, outIsSettable);
      break;
    ...
    case kObjectID_DataSource_Output_Master:
    case kObjectID_DataDestination_PlayThru_Master:
      theAnswer = AudioLoopback_IsControlPropertySettable(inDriver, inObjectID, inClientProcessID,
                                                     inAddress, outIsSettable);
      break;
...
static OSStatus AudioLoopback_GetPropertyDataSize(AudioServerPlugInDriverRef inDriver,
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetPropertyDataSize: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPropertyDataSize: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPropertyDataSize: no place to put the return value");
  ...
  switch (inObjectID) {
    case kObjectID_PlugIn:
      theAnswer =
          AudioLoopback_GetPlugInPropertyDataSize(inDriver, inObjectID, inClientProcessID, inAddress,
                                             inQualifierDataSize, inQualifierData, outDataSize);
      break;

    case kObjectID_Box:
      theAnswer =
          AudioLoopback_GetBoxPropertyDataSize(inDriver, inObjectID, inClientProcessID, inAddress,
                                          inQualifierDataSize, inQualifierData, outDataSize);
      break;

    case kObjectID_Device:
      theAnswer =
          AudioLoopback_GetDevicePropertyDataSize(inDriver, inObjectID, inClientProcessID, inAddress,
                                             inQualifierDataSize, inQualifierData, outDataSize);
      break;
    ...
    case kObjectID_Stream_Output:
      theAnswer =
          AudioLoopback_GetStreamPropertyDataSize(inDriver, inObjectID, inClientProcessID, inAddress,
                                             inQualifierDataSize, inQualifierData, outDataSize);
      break;
    ...
    case kObjectID_DataDestination_PlayThru_Master:
      theAnswer =
          AudioLoopback_GetControlPropertyDataSize(inDriver, inObjectID, inClientProcessID, inAddress,
                                              inQualifierDataSize, inQualifierData, outDataSize);
      break;
...
static OSStatus AudioLoopback_GetPropertyData(AudioServerPlugInDriverRef inDriver,
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPropertyData: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPropertyData: no place to put the return value size");
  FailWithAction(outData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPropertyData: no place to put the return value");
  ...
  switch (inObjectID) {
    case kObjectID_PlugIn:
      theAnswer = AudioLoopback_GetPlugInPropertyData(inDriver, inObjectID, inClientProcessID, inAddress,
                                                 inQualifierDataSize, inQualifierData, inDataSize,
                                                 outDataSize, outData);
      break;

    case kObjectID_Box:
      theAnswer = AudioLoopback_GetBoxPropertyData(inDriver, inObjectID, inClientProcessID, inAddress,
                                              inQualifierDataSize, inQualifierData, inDataSize,
                                              outDataSize, outData);
      break;

    case kObjectID_Device:
      theAnswer = AudioLoopback_GetDevicePropertyData(inDriver, inObjectID, inClientProcessID, inAddress,
                                                 inQualifierDataSize, inQualifierData, inDataSize,
                                                 outDataSize, outData);
      break;
    ...
    case kObjectID_Stream_Input:
    case kObjectID_Stream_Output:
      theAnswer = AudioLoopback_GetStreamPropertyData(inDriver, inObjectID, inClientProcessID, inAddress,
                                                 inQualifierDataSize, inQualifierData, inDataSize,
                                                 outDataSize, outData);
      break;
    ...
    case kObjectID_DataSource_Output_Master:
    case kObjectID_DataDestination_PlayThru_Master:
      theAnswer = AudioLoopback_GetControlPropertyData(inDriver, inObjectID, inClientProcessID,
...
static OSStatus AudioLoopback_SetPropertyData(AudioServerPlugInDriverRef inDriver,
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_SetPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetPropertyData: no address");
  ...
  switch (inObjectID) {
    case kObjectID_PlugIn:
      theAnswer = AudioLoopback_SetPlugInPropertyData(
          inDriver, inObjectID, inClientProcessID, inAddress, inQualifierDataSize, inQualifierData,
          inDataSize, inData, &theNumberPropertiesChanged, theChangedAddresses);
      break;

    case kObjectID_Box:
      theAnswer = AudioLoopback_SetBoxPropertyData(
          inDriver, inObjectID, inClientProcessID, inAddress, inQualifierDataSize, inQualifierData,
          inDataSize, inData, &theNumberPropertiesChanged, theChangedAddresses);
      break;

    case kObjectID_Device:
      theAnswer = AudioLoopback_SetDevicePropertyData(
          inDriver, inObjectID, inClientProcessID, inAddress, inQualifierDataSize, inQualifierData,
          inDataSize, inData, &theNumberPropertiesChanged, theChangedAddresses);
      break;
    ...
    case kObjectID_Stream_Input:
    case kObjectID_Stream_Output:
      theAnswer = AudioLoopback_SetStreamPropertyData(
          inDriver, inObjectID, inClientProcessID, inAddress, inQualifierDataSize, inQualifierData,
          inDataSize, inData, &theNumberPropertiesChanged, theChangedAddresses);
      break;
    ...
    case kObjectID_DataSource_Output_Master:
    case kObjectID_DataDestination_PlayThru_Master:
      theAnswer = AudioLoopback_SetControlPropertyData(
          inDriver, inObjectID, inClientProcessID, inAddress, inQualifierDataSize, inQualifierData,
          inDataSize, inData, &theNumberPropertiesChanged, theChangedAddresses);
      break;
  ...
  if (theNumberPropertiesChanged > 0) {
    gPlugIn_Host->PropertiesChanged(gPlugIn_Host, inObjectID, theNumberPropertiesChanged,
                                    theChangedAddresses);
  }
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done,
         "AudioLoopback_HasPlugInProperty: bad driver reference");
  FailIf(inAddress == NULL, Done, "AudioLoopback_HasPlugInProperty: no address");
  FailIf(inObjectID != kObjectID_PlugIn, Done,
         "AudioLoopback_HasPlugInProperty: not the plug-in object");

  //	Note that for each object, this driver implements all the required properties plus a few
  //	extras that are useful but not required. There is more detailed commentary about each
  //	property in the AudioLoopback_GetPlugInPropertyData() method.
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_IsPlugInPropertySettable: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsPlugInPropertySettable: no address");
  FailWithAction(outIsSettable == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsPlugInPropertySettable: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_PlugIn, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_IsPlugInPropertySettable: not the plug-in object");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetPlugInPropertyDataSize: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPlugInPropertyDataSize: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPlugInPropertyDataSize: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_PlugIn, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetPlugInPropertyDataSize: not the plug-in object");
  ...
  switch (inAddress->mSelector) {
    ...
    case kPlugIn_CustomPropertyID:
      FailWithAction(inQualifierDataSize != sizeof(CFStringRef),
                     ...
                     "AudioLoopback_GetPlugInPropertyDataSize: the qualifier is the wrong size for "
                     "kPlugIn_CustomPropertyID");
      FailWithAction(
          inQualifierData == NULL, theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "AudioLoopback_GetPlugInPropertyDataSize: no qualifier for kPlugIn_CustomPropertyID");
      DebugMsg("AudioLoopback_GetPlugInPropertyDataSize: the qualifier passed to us was:");
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetPlugInPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPlugInPropertyData: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPlugInPropertyData: no place to put the return value size");
  FailWithAction(outData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetPlugInPropertyData: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_PlugIn, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetPlugInPropertyData: not the plug-in object");
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetPlugInPropertyData: not enough space for the return value of "
    ...
    case kAudioObjectPropertyClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetPlugInPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyClass for the plug-in");
    ...
    case kAudioObjectPropertyOwner:
      ...
      FailWithAction(inDataSize < sizeof(AudioObjectID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetPlugInPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyOwner for the plug-in");
    ...
    case kAudioPlugInPropertyTranslateUIDToBox:
      ...
      FailWithAction(inQualifierDataSize != sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetPlugInPropertyData: the qualifier is the wrong size for "
                     "kAudioPlugInPropertyTranslateUIDToBox");
      FailWithAction(
          inQualifierData == NULL, theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "AudioLoopback_GetPlugInPropertyData: no qualifier for kAudioPlugInPropertyTranslateUIDToBox");
      if (CFStringCompare(*((CFStringRef*)inQualifierData), CFSTR(kBox_UID), 0) ==
          kCFCompareEqualTo) {
    ...
    case kAudioPlugInPropertyTranslateUIDToDevice:
      ...
      FailWithAction(inQualifierData == NULL, theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetPlugInPropertyData: no qualifier for "
                     "kAudioPlugInPropertyTranslateUIDToDevice");
      if (CFStringCompare(*((CFStringRef*)inQualifierData), CFSTR(kDevice_UID), 0) ==
          kCFCompareEqualTo) {
    ...
    case kPlugIn_CustomPropertyID:
      ...
      FailWithAction(inQualifierDataSize != sizeof(CFPropertyListRef),
                     ...
                     "kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierData == NULL, theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetPlugInPropertyData: no qualifier for kPlugIn_CustomPropertyID");
      DebugMsg("AudioLoopback_GetPlugInPropertyData: the qualifier passed to us was:");
      CFShow(*((CFPropertyListRef*)inQualifierData));
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData, inDataSize, inData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_SetPlugInPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetPlugInPropertyData: no address");
  FailWithAction(
      outNumberPropertiesChanged == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
      "AudioLoopback_SetPlugInPropertyData: no place to return the number of properties that changed");
  FailWithAction(outChangedAddresses == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetPlugInPropertyData: no place to return the properties that changed");
  FailWithAction(inObjectID != kObjectID_PlugIn, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_SetPlugInPropertyData: not the plug-in object");
  ...
  *outNumberPropertiesChanged = 0;
  ...
  switch (inAddress->mSelector) {
    case kPlugIn_CustomPropertyID:
      FailWithAction(inDataSize != sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_SetPlugInPropertyData: not enough space for the return value of "
                     "kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierDataSize != sizeof(CFPropertyListRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_SetPlugInPropertyData: the qualifier is the wrong size for "
                     "kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierData == NULL, theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_SetPlugInPropertyData: no qualifier for kPlugIn_CustomPropertyID");
      DebugMsg("AudioLoopback_SetPlugInPropertyData: the qualifier passed to us was:");
      CFShow(*((CFPropertyListRef*)inQualifierData));
      DebugMsg("AudioLoopback_SetPlugInPropertyData: the data passed to us was:");
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done,
         "AudioLoopback_HasBoxProperty: bad driver reference");
  FailIf(inAddress == NULL, Done, "AudioLoopback_HasBoxProperty: no address");
  FailIf(inObjectID != kObjectID_Box, Done, "AudioLoopback_HasBoxProperty: not the box object");

  //	Note that for each object, this driver implements all the required properties plus a few
  //	extras that are useful but not required. There is more detailed commentary about each
  //	property in the AudioLoopback_GetBoxPropertyData() method.
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
    case kAudioObjectPropertyClass:
    case kAudioObjectPropertyOwner:
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_IsBoxPropertySettable: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsBoxPropertySettable: no address");
  FailWithAction(outIsSettable == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsBoxPropertySettable: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_Box, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_IsBoxPropertySettable: not the plug-in object");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetBoxPropertyDataSize: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetBoxPropertyDataSize: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetBoxPropertyDataSize: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_Box, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetBoxPropertyDataSize: not the plug-in object");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetBoxPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetBoxPropertyData: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetBoxPropertyData: no place to put the return value size");
  FailWithAction(outData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetBoxPropertyData: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_Box, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetBoxPropertyData: not the plug-in object");
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyBaseClass for the box");
      *((AudioClassID*)outData) = kAudioObjectClassID;
      *outDataSize = sizeof(AudioClassID);
      break;
    ...
    case kAudioObjectPropertyClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyClass for the box");
    ...
    case kAudioObjectPropertyOwner:
      ...
      FailWithAction(inDataSize < sizeof(AudioObjectID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyOwner for the box");
    ...
    case kAudioObjectPropertyIdentify:
      ...
      FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyIdentify for the box");
      *((UInt32*)outData) = 0;
      *outDataSize = sizeof(UInt32);
      break;
    ...
    case kAudioBoxPropertyHasAudio:
      ...
      FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioBoxPropertyHasAudio for the box");
      *((UInt32*)outData) = 1;
      *outDataSize = sizeof(UInt32);
      break;
    ...
    case kAudioBoxPropertyHasVideo:
      ...
      FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioBoxPropertyHasVideo for the box");
      *((UInt32*)outData) = 0;
      *outDataSize = sizeof(UInt32);
      break;
    ...
    case kAudioBoxPropertyHasMIDI:
      ...
      FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioBoxPropertyHasMIDI for the box");
      *((UInt32*)outData) = 0;
      *outDataSize = sizeof(UInt32);
      break;
    ...
    case kAudioBoxPropertyIsProtected:
      ...
      FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioBoxPropertyIsProtected for the box");
      *((UInt32*)outData) = 0;
      *outDataSize = sizeof(UInt32);
      break;
    ...
    case kAudioBoxPropertyAcquired:
      ...
      FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
                     "kAudioBoxPropertyAcquired for the box");
      pthread_mutex_lock(&gPlugIn_StateMutex);
    ...
    case kAudioBoxPropertyAcquisitionFailed:
      //	This is used for notifications to say when an attempt to acquire a device has
      // failed.
      FailWithAction(inDataSize < sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetBoxPropertyData: not enough space for the return value of "
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData, inDataSize, inData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_SetBoxPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetBoxPropertyData: no address");
  FailWithAction(
      outNumberPropertiesChanged == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
      "AudioLoopback_SetBoxPropertyData: no place to return the number of properties that changed");
  FailWithAction(outChangedAddresses == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetBoxPropertyData: no place to return the properties that changed");
  FailWithAction(inObjectID != kObjectID_Box, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_SetBoxPropertyData: not the box object");
  ...
  *outNumberPropertiesChanged = 0;
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyName:
      ...
      {
        FailWithAction(
            inDataSize != sizeof(CFStringRef), theAnswer = kAudioHardwareBadPropertySizeError, Done,
            "AudioLoopback_SetBoxPropertyData: wrong size for the data for kAudioObjectPropertyName");
        FailWithAction(inData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                       "AudioLoopback_SetBoxPropertyData: no data to set for kAudioObjectPropertyName");
        ...
        *outNumberPropertiesChanged = 1;
        ...
        outChangedAddresses[0].mElement = kAudioObjectPropertyElementName;
      }
      break;

    case kAudioObjectPropertyIdentify:
      //	since we don't have any actual hardware to flash, we will schedule a notificaiton
      // for 	this property off into the future as a testing thing. Note that a real
      // implementation 	of this property should only send the notificaiton if the hardware
      // wants the app to 	flash it's UI for the device.
      {
        syslog(
            LOG_NOTICE,
        ...
        FailWithAction(inDataSize != sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                       Done,
                       "AudioLoopback_SetBoxPropertyData: wrong size for the data for "
        ...
        dispatch_after(dispatch_time(0, 2ULL * 1000ULL * 1000ULL * 1000ULL),
                       dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^() {
                         ...
                         gPlugIn_Host->PropertiesChanged(gPlugIn_Host, kObjectID_Box, 1,
                                                         &theAddress);
                       });
      }
      break;
    ...
    case kAudioBoxPropertyAcquired:
      //	When the box is acquired, it means the contents, namely the device, are available to
      // the system
      {
        FailWithAction(
            inDataSize != sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError, Done,
            "AudioLoopback_SetBoxPropertyData: wrong size for the data for kAudioBoxPropertyAcquired");
        pthread_mutex_lock(&gPlugIn_StateMutex);
        if (gBox_Acquired != (*((UInt32*)inData) != 0)) {
          ...
          *outNumberPropertiesChanged = 2;
          ...
          outChangedAddresses[1].mElement = kAudioObjectPropertyElementName;
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done,
         "AudioLoopback_HasDeviceProperty: bad driver reference");
  FailIf(inAddress == NULL, Done, "AudioLoopback_HasDeviceProperty: no address");
  FailIf(inObjectID != kObjectID_Device, Done, "AudioLoopback_HasDeviceProperty: not the device object");

  //	Note that for each object, this driver implements all the required properties plus a few
  //	extras that are useful but not required. There is more detailed commentary about each
  //	property in the AudioLoopback_GetDevicePropertyData() method.
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
    case kAudioObjectPropertyClass:
    case kAudioObjectPropertyOwner:
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_IsDevicePropertySettable: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsDevicePropertySettable: no address");
  FailWithAction(outIsSettable == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsDevicePropertySettable: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_IsDevicePropertySettable: not the device object");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetDevicePropertyDataSize: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetDevicePropertyDataSize: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetDevicePropertyDataSize: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetDevicePropertyDataSize: not the device object");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetDevicePropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetDevicePropertyData: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetDevicePropertyData: no place to put the return value size");
  FailWithAction(outData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetDevicePropertyData: no place to put the return value");
  FailWithAction(inObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetDevicePropertyData: not the device object");
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetDevicePropertyData: not enough space for the return value of "
    ...
    case kAudioObjectPropertyClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetDevicePropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyClass for the device");
    ...
    case kAudioObjectPropertyOwner:
      ...
      FailWithAction(inDataSize < sizeof(AudioObjectID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetDevicePropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyOwner for the device");
      *((AudioObjectID*)outData) = kObjectID_PlugIn;
      *outDataSize = sizeof(AudioObjectID);
      break;
    ...
    case kAudioObjectPropertyName:
      ...
      *((CFStringRef*)outData) = CFSTR("AudioLoopback");
      *outDataSize = sizeof(CFStringRef);
      break;
    ...
    case kAudioObjectPropertyManufacturer:
      ...
      *((CFStringRef*)outData) = CFSTR("Agora Inc.");
      *outDataSize = sizeof(CFStringRef);
      break;
    ...
    case kAudioObjectPropertyElementName:
      ...
      FailWithAction(inDataSize < sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetDevicePropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyElementName for the device");
      ...
      for (theItemIndex = 0; theItemIndex < theNumberItemsToFetch; ++theItemIndex) {
        if (theItemIndex < 3) {
          ...
          ((AudioObjectID*)outData)[theItemIndex] =
              kObjectID_Volume_Output_Master + (theItemIndex - 3);
        }
      }
    ...
    case kAudioDevicePropertyNominalSampleRate:
      ...
      FailWithAction(inDataSize < sizeof(Float64), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetDevicePropertyData: not enough space for the return value of "
    ...
    case kAudioDevicePropertyPreferredChannelLayout:
      ...
      //	by default. For this device, we return a stereo ACL.
      {
        ...
        UInt32 theACLSize = offsetof(AudioChannelLayout, mChannelDescriptions) +
                            (2 * sizeof(AudioChannelDescription));
        FailWithAction(inDataSize < theACLSize, theAnswer = kAudioHardwareBadPropertySizeError,
                       Done,
                       "AudioLoopback_GetDevicePropertyData: not enough space for the return value of "
    ...
    case kAudioDevicePropertyIcon: {
      ...
      FailWithAction(inDataSize < sizeof(CFURLRef), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_GetDevicePropertyData: not enough space for the return value of "
                     "kAudioDevicePropertyDeviceUID for the device");
      CFBundleRef theBundle = CFBundleGetBundleWithIdentifier(CFSTR(kPlugIn_BundleID));
      FailWithAction(theBundle == NULL, theAnswer = kAudioHardwareUnspecifiedError, Done,
                     "AudioLoopback_GetDevicePropertyData: could not get the plug-in bundle for "
                     "kAudioDevicePropertyIcon");
      CFURLRef theURL = CFBundleCopyResourceURL(theBundle, CFSTR("DeviceIcon.icns"), NULL, NULL);
      FailWithAction(
          theURL == NULL, theAnswer = kAudioHardwareUnspecifiedError, Done,
          "AudioLoopback_GetDevicePropertyData: could not get the URL for kAudioDevicePropertyIcon");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_SetDevicePropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetDevicePropertyData: no address");
  FailWithAction(
      outNumberPropertiesChanged == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
      "AudioLoopback_SetDevicePropertyData: no place to return the number of properties that changed");
  FailWithAction(outChangedAddresses == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetDevicePropertyData: no place to return the properties that changed");
  FailWithAction(inObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_SetDevicePropertyData: not the device object");
  ...
  *outNumberPropertiesChanged = 0;
  ...
  switch (inAddress->mSelector) {
    case kAudioDevicePropertyNominalSampleRate:
      ...
      FailWithAction(inDataSize != sizeof(Float64), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_SetDevicePropertyData: wrong size for the data for "
                     "kAudioDevicePropertyNominalSampleRate");
      FailWithAction(
          (*((const Float64*)inData) != 44100.0) && (*((const Float64*)inData) != 48000.0),
          theAnswer = kAudioHardwareIllegalOperationError, Done,
          "AudioLoopback_SetDevicePropertyData: unsupported value for "
          "kAudioDevicePropertyNominalSampleRate");
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done,
         "AudioLoopback_HasStreamProperty: bad driver reference");
  FailIf(inAddress == NULL, Done, "AudioLoopback_HasStreamProperty: no address");
  FailIf((inObjectID != kObjectID_Stream_Input) && (inObjectID != kObjectID_Stream_Output), Done,
         "AudioLoopback_HasStreamProperty: not a stream object");

  //	Note that for each object, this driver implements all the required properties plus a few
  //	extras that are useful but not required. There is more detailed commentary about each
  //	property in the AudioLoopback_GetStreamPropertyData() method.
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
    case kAudioObjectPropertyClass:
    case kAudioObjectPropertyOwner:
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_IsStreamPropertySettable: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsStreamPropertySettable: no address");
  FailWithAction(outIsSettable == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsStreamPropertySettable: no place to put the return value");
  FailWithAction((inObjectID != kObjectID_Stream_Input) && (inObjectID != kObjectID_Stream_Output),
                 theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_IsStreamPropertySettable: not a stream object");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetStreamPropertyDataSize: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetStreamPropertyDataSize: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetStreamPropertyDataSize: no place to put the return value");
  FailWithAction((inObjectID != kObjectID_Stream_Input) && (inObjectID != kObjectID_Stream_Output),
                 theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetStreamPropertyDataSize: not a stream object");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetStreamPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetStreamPropertyData: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetStreamPropertyData: no place to put the return value size");
  FailWithAction(outData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetStreamPropertyData: no place to put the return value");
  FailWithAction((inObjectID != kObjectID_Stream_Input) && (inObjectID != kObjectID_Stream_Output),
                 theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_GetStreamPropertyData: not a stream object");
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetStreamPropertyData: not enough space for the return value of "
    ...
    case kAudioObjectPropertyClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetStreamPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyClass for the stream");
    ...
    case kAudioObjectPropertyOwner:
      ...
      FailWithAction(inDataSize < sizeof(AudioObjectID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetStreamPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyOwner for the stream");
      *((AudioObjectID*)outData) = kObjectID_Device;
      *outDataSize = sizeof(AudioObjectID);
      break;
    ...
    case kAudioObjectPropertyName:
      ...
      FailWithAction(inDataSize < sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "AudioLoopback_GetStreamPropertyData: not enough space for the return value of "
                     "kAudioObjectPropertyName for the stream");
      *((CFStringRef*)outData) = (inObjectID == kObjectID_Stream_Input) ? CFSTR("InputStreamName")
    ...
    case kAudioStreamPropertyIsActive:
      ...
      pthread_mutex_lock(&gPlugIn_StateMutex);
      *((UInt32*)outData) =
          (inObjectID == kObjectID_Stream_Input) ? gStream_Input_IsActive : gStream_Output_IsActive;
      pthread_mutex_unlock(&gPlugIn_StateMutex);
      *outDataSize = sizeof(UInt32);
      break;
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_SetStreamPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetStreamPropertyData: no address");
  FailWithAction(
      outNumberPropertiesChanged == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
      "AudioLoopback_SetStreamPropertyData: no place to return the number of properties that changed");
  FailWithAction(outChangedAddresses == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetStreamPropertyData: no place to return the properties that changed");
  FailWithAction((inObjectID != kObjectID_Stream_Input) && (inObjectID != kObjectID_Stream_Output),
                 theAnswer = kAudioHardwareBadObjectError, Done,
                 "AudioLoopback_SetStreamPropertyData: not a stream object");
  ...
  *outNumberPropertiesChanged = 0;
  ...
  switch (inAddress->mSelector) {
    case kAudioStreamPropertyIsActive:
      ...
      FailWithAction(inDataSize != sizeof(UInt32), theAnswer = kAudioHardwareBadPropertySizeError,
                     Done,
                     "AudioLoopback_SetStreamPropertyData: wrong size for the data for "
                     "kAudioDevicePropertyNominalSampleRate");
      pthread_mutex_lock(&gPlugIn_StateMutex);
    ...
    case kAudioStreamPropertyPhysicalFormat:
      ...
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mFormatID != kAudioFormatLinearPCM,
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "AudioLoopback_SetStreamPropertyData: unsupported format ID for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mFormatFlags !=
              (kAudioFormatFlagIsFloat | kAudioFormatFlagsNativeEndian | kAudioFormatFlagIsPacked),
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "AudioLoopback_SetStreamPropertyData: unsupported format flags for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(((const AudioStreamBasicDescription*)inData)->mBytesPerPacket != 8,
                     theAnswer = kAudioDeviceUnsupportedFormatError, Done,
                     "AudioLoopback_SetStreamPropertyData: unsupported bytes per packet for "
                     "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(((const AudioStreamBasicDescription*)inData)->mFramesPerPacket != 1,
                     theAnswer = kAudioDeviceUnsupportedFormatError, Done,
                     "AudioLoopback_SetStreamPropertyData: unsupported frames per packet for "
                     "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(((const AudioStreamBasicDescription*)inData)->mBytesPerFrame != 8,
                     theAnswer = kAudioDeviceUnsupportedFormatError, Done,
                     "AudioLoopback_SetStreamPropertyData: unsupported bytes per frame for "
                     "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(((const AudioStreamBasicDescription*)inData)->mChannelsPerFrame != 2,
                     theAnswer = kAudioDeviceUnsupportedFormatError, Done,
                     "AudioLoopback_SetStreamPropertyData: unsupported channels per frame for "
                     "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(((const AudioStreamBasicDescription*)inData)->mBitsPerChannel != 32,
                     theAnswer = kAudioDeviceUnsupportedFormatError, Done,
                     "AudioLoopback_SetStreamPropertyData: unsupported bits per channel for "
                     "kAudioStreamPropertyPhysicalFormat");
      FailWithAction((((const AudioStreamBasicDescription*)inData)->mSampleRate != 44100.0) &&
                     ...
                     theAnswer = kAudioHardwareIllegalOperationError, Done,
                     "AudioLoopback_SetStreamPropertyData: unsupported sample rate for "
                     "kAudioStreamPropertyPhysicalFormat");

      //	If we made it this far, the requested format is something we support, so make sure
      // the sample rate is actually different
      pthread_mutex_lock(&gPlugIn_StateMutex);
      theOldSampleRate = gDevice_SampleRate;
      pthread_mutex_unlock(&gPlugIn_StateMutex);
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailIf(inDriver != gAudioServerPlugInDriverRef, Done,
         "AudioLoopback_HasControlProperty: bad driver reference");
  FailIf(inAddress == NULL, Done, "AudioLoopback_HasControlProperty: no address");

  //	Note that for each object, this driver implements all the required properties plus a few
  //	extras that are useful but not required. There is more detailed commentary about each
  //	property in the AudioLoopback_GetControlPropertyData() method.
  switch (inObjectID) {
    case kObjectID_Volume_Input_Master:
    case kObjectID_Volume_Output_Master:
      switch (inAddress->mSelector) {
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_IsControlPropertySettable: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsControlPropertySettable: no address");
  FailWithAction(outIsSettable == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_IsControlPropertySettable: no place to put the return value");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetControlPropertyDataSize: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetControlPropertyDataSize: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetControlPropertyDataSize: no place to put the return value");
...
#pragma unused(inClientProcessID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetControlPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetControlPropertyData: no address");
  FailWithAction(outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetControlPropertyData: no place to put the return value size");
  FailWithAction(outData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_GetControlPropertyData: no place to put the return value");
    ...
    case kObjectID_Volume_Output_Master:
      switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
          ...
          FailWithAction(inDataSize < sizeof(AudioClassID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyBaseClass for the volume control");
        ...
        case kAudioObjectPropertyClass:
          ...
          FailWithAction(inDataSize < sizeof(AudioClassID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyClass for the volume control");
        ...
        case kAudioObjectPropertyOwner:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyOwner for the volume control");
          *((AudioObjectID*)outData) = kObjectID_Device;
          *outDataSize = sizeof(AudioObjectID);
          break;
        ...
        case kAudioControlPropertyScope:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioControlPropertyScope for the volume control");
        ...
        case kAudioControlPropertyElement:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioControlPropertyElement for the volume control");
          *((AudioObjectPropertyElement*)outData) = kAudioObjectPropertyElementName;
          *outDataSize = sizeof(AudioObjectPropertyElement);
          break;
        ...
        case kAudioLevelControlPropertyDecibelValue:
          ...
          *((Float32*)outData) =
              kVolume_MinDB + (*((Float32*)outData) * (kVolume_MaxDB - kVolume_MinDB));
        ...
        case kAudioLevelControlPropertyDecibelRange:
          ...
          FailWithAction(inDataSize < sizeof(AudioValueRange),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioLevelControlPropertyDecibelRange for the volume control");
      ...
      switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
          ...
          FailWithAction(inDataSize < sizeof(AudioClassID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyBaseClass for the mute control");
        ...
        case kAudioObjectPropertyClass:
          ...
          FailWithAction(inDataSize < sizeof(AudioClassID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyClass for the mute control");
        ...
        case kAudioObjectPropertyOwner:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyOwner for the mute control");
          *((AudioObjectID*)outData) = kObjectID_Device;
          *outDataSize = sizeof(AudioObjectID);
          break;
        ...
        case kAudioControlPropertyScope:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioControlPropertyScope for the mute control");
        ...
        case kAudioControlPropertyElement:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioControlPropertyElement for the mute control");
          *((AudioObjectPropertyElement*)outData) = kAudioObjectPropertyElementName;
          *outDataSize = sizeof(AudioObjectPropertyElement);
          break;
        ...
        case kAudioBooleanControlPropertyValue:
          ...
          pthread_mutex_lock(&gPlugIn_StateMutex);
          *((UInt32*)outData) = (inObjectID == kObjectID_Mute_Input_Master)
      ...
      switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
          ...
          FailWithAction(inDataSize < sizeof(AudioClassID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyBaseClass for the data source control");
        ...
        case kAudioObjectPropertyClass:
          ...
          FailWithAction(inDataSize < sizeof(AudioClassID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyClass for the data source control");
        ...
        case kAudioObjectPropertyOwner:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioObjectPropertyOwner for the data source control");
          *((AudioObjectID*)outData) = kObjectID_Device;
          *outDataSize = sizeof(AudioObjectID);
          break;
        ...
        case kAudioControlPropertyScope:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioControlPropertyScope for the data source control");
          switch (inObjectID) {
            case kObjectID_DataSource_Input_Master:
        ...
        case kAudioControlPropertyElement:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioControlPropertyElement for the data source control");
          *((AudioObjectPropertyElement*)outData) = kAudioObjectPropertyElementName;
          *outDataSize = sizeof(AudioObjectPropertyElement);
          break;
        ...
        case kAudioSelectorControlPropertyCurrentItem:
          ...
          FailWithAction(inDataSize < sizeof(UInt32),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioSelectorControlPropertyCurrentItem for the data source control");
          pthread_mutex_lock(&gPlugIn_StateMutex);
          switch (inObjectID) {
            case kObjectID_DataSource_Input_Master:
        ...
        case kAudioSelectorControlPropertyItemName:
          ...
          FailWithAction(inDataSize < sizeof(CFStringRef),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: not enough space for the return value "
                         "of kAudioSelectorControlPropertyItemName for the data source control");
          FailWithAction(inQualifierDataSize != sizeof(UInt32),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_GetControlPropertyData: wrong size for the qualifier of "
                         "kAudioSelectorControlPropertyItemName for the data source control");
          FailWithAction(*((const UInt32*)inQualifierData) >= kDataSource_NumberItems,
                         theAnswer = kAudioHardwareIllegalOperationError, Done,
                         "AudioLoopback_GetControlPropertyData: the item in the qualifier is not valid "
                         "for kAudioSelectorControlPropertyItemName for the data source control");
          *((CFStringRef*)outData) = CFStringCreateWithFormat(
              NULL, NULL, CFSTR(kDataSource_ItemNamePattern), *((const UInt32*)inQualifierData));
          *outDataSize = sizeof(CFStringRef);
          break;
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_SetControlPropertyData: bad driver reference");
  FailWithAction(inAddress == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetControlPropertyData: no address");
  FailWithAction(
      outNumberPropertiesChanged == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
      "AudioLoopback_SetControlPropertyData: no place to return the number of properties that changed");
  FailWithAction(outChangedAddresses == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "AudioLoopback_SetControlPropertyData: no place to return the properties that changed");
    ...
    case kObjectID_Volume_Output_Master:
      switch (inAddress->mSelector) {
        case kAudioLevelControlPropertyScalarValue:
          ...
          FailWithAction(inDataSize != sizeof(Float32),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "AudioLoopback_SetControlPropertyData: wrong size for the data for "
      ...
      switch (inAddress->mSelector) {
        case kAudioSelectorControlPropertyCurrentItem:
          ...
          FailWithAction(*((const UInt32*)inData) >= kDataSource_NumberItems,
                         theAnswer = kAudioHardwareIllegalOperationError, Done,
                         "AudioLoopback_SetControlPropertyData: requested item not in available items "
                         "list for kAudioSelectorControlPropertyCurrentItem");
          pthread_mutex_lock(&gPlugIn_StateMutex);
          switch (inObjectID) {
            case kObjectID_DataSource_Input_Master: {
...
#pragma unused(inClientID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_StartIO: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_StartIO: bad device ID");
...
#pragma unused(inClientID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_StopIO: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_StopIO: bad device ID");
...
#pragma unused(inClientID)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetZeroTimeStamp: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_GetZeroTimeStamp: bad device ID");
  ...
  *outHostTime =
      gDevice_AnchorHostTime + (((Float64)gDevice_NumberTimeStamps) * theHostTicksPerRingBuffer);
  *outSeed = 1;
...
#pragma unused(inClientID)
  ...
  //    check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_WillDoIOOperation: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_WillDoIOOperation: bad device ID");
...
#pragma unused(inClientID, inOperationID, inIOBufferFrameSize, inIOCycleInfo)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_BeginIOOperation: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_BeginIOOperation: bad device ID");

Done:
  return theAnswer;
}

static void ReadInputData(UInt32 inIOBufferFrameSize, Float64 inSampleTime, void* outBuffer) {
  ...
  CARingBufferError err = gLoopbackRingBuffer.Fetch(
      &abl, inIOBufferFrameSize, static_cast<CARingBuffer::SampleTime>(inSampleTime));
...
static void WriteOutputData(UInt32 inIOBufferFrameSize, Float64 inSampleTime,
                            const void* inBuffer) {
  // Wrap the provided buffer in an AudioBufferList.
  AudioBufferList abl = {
      .mNumberBuffers = 1,
  ...
  CARingBufferError err = gLoopbackRingBuffer.Store(
      &abl, inIOBufferFrameSize, static_cast<CARingBuffer::SampleTime>(inSampleTime));
}
...
#pragma unused(inClientID, inIOCycleInfo, ioSecondaryBuffer)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_DoIOOperation: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_DoIOOperation: bad device ID");
  FailWithAction(
      (inStreamObjectID != kObjectID_Stream_Input) && (inStreamObjectID != kObjectID_Stream_Output),
      theAnswer = kAudioHardwareBadObjectError, Done, "AudioLoopback_DoIOOperation: bad stream ID");
...
#pragma unused(inClientID, inOperationID, inIOBufferFrameSize, inIOCycleInfo)
  ...
  //	check the arguments
  FailWithAction(inDriver != gAudioServerPlugInDriverRef, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_EndIOOperation: bad driver reference");
  FailWithAction(inDeviceObjectID != kObjectID_Device, theAnswer = kAudioHardwareBadObjectError,
                 Done, "AudioLoopback_EndIOOperation: bad device ID");
